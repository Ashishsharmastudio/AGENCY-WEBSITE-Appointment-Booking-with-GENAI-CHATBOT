...
export async function POST(req: Request) {
  try {
    ...
    // accept all form fields from the booking page
    const { name, mobile, email, address, service, date, message, fileUrl } = body;

    if (!service || !date) {
      return NextResponse.json({ error: "Service and date are required" }, { status: 400 });
    }

    // store contact fields under a `user` object so the admin UI (which reads `a.user?.name`) works
    const appointment = {
      user: {
        name: name || "",
        email: email || "",
